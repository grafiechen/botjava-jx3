...
### 2026-08-25 MongoDB 指定字段图片查询

- 新增群指令 `查询信息 区服 角色名 Mongo字段`，继续使用统一 `/指令` 规范化和 30 秒外部调用冷却；仅允许查询当前群内发送者本人已绑定的区服与角色名。
- 查询仍由 `LuaRoleStatusStore` 只按配置的“服务器 + 角色名”精确匹配，不改变 MongoDB 索引，不新增文档，也不把字段名拼入查询条件；同名角色命中多条文档时保留记录分组。
- 新增 `RoleFieldQueryAccessPolicy`。账号、全局 ID、角色 ID、OpenID、凭证和个人联系方式等内置敏感字段始终禁止，配置可追加黑名单；白名单开关和允许字段列表已经预留，当前默认关闭。
- 单值、日期、布尔值统一转换为一项列表，数组和集合逐项生成图片行；null、空文本、空集合、缺失字段以及仅含对象的数据统一返回“未查询到数据”。对象不递归展开，防止嵌套内部字段越权展示。
- 新增 Vue 模板 `src/main/resources/static/查询信息.html`，展示区服、角色、字段名、Mongo 匹配记录分组和列表项；图片总项数默认限制为 200，可通过配置调整。
- MongoDB 未启用、匹配记录超过上限和驱动异常均返回稳定文本，不向 QQ WebSocket 处理线程抛出；服务端错误日志保留完整堆栈且不记录被查询字段值。
- 增加命令解析、角色归属、标量/列表归一化、空结果、敏感字段、配置黑白名单、Mongo 异常和 Playwright 模板渲染测试。
- 最终全量 `mvn test` 共 672 项通过，失败 0、错误 0、跳过 0；其中 HTML/Vue 浏览器渲染测试 24 项，`查询信息` 样例图片为 `1280 × 720`。

### 2026-08-25 MongoDB 查询字段别名展开

- 复用 PostgreSQL `script_status_field.display_name` 与 `group_name` 作为查询别名，不新增表或迁移；显示名可定位对应字段，分组名会按照现有分组、排序号和 ID 顺序稳定展开同组的全部启用 Mongo 顶层字段。
- `查询信息 区服 角色名 名称` 先匹配真实 `mongo_field_name`，精确命中时只查询该字段；否则匹配 `display_name` 或 `group_name` 并读取全部对应字段；都未命中时继续按原始顶层字段查询，保持旧指令兼容。
- 名称匹配使用 Unicode NFKC、忽略大小写，并忽略空白、点号、下划线、连字符和间隔点差异。别名展开后逐字段执行敏感字段、黑名单和可选白名单校验，任一字段越权则整次拒绝。
- `查询信息.html` 数据结构调整为 `records[].fields[].items[]`，同一张图片按脚本记录和真实字段分层展示；总项数上限对全部展开字段合计生效。
- 增加精确字段优先、别名一对多、未知名称回退、输入标准化、展开后敏感字段拒绝以及多字段 Vue 图片渲染测试。
- 本轮全量 `mvn test` 共 676 项通过，失败 0、错误 0、跳过 0；其中 HTML/Vue 浏览器渲染测试 24 项，别名多字段样例图片为 `1280 × 720`。

### 2026-08-25 查询信息紧凑表格图片

- `查询信息.html` 改为类 Excel 的字段表格：字段显示名位于单元格上方，字段值位于下方；同一条脚本记录的字段按水平方向排列，每行最多 5 项，第 6 项起自动换行。
- 图片宽度依据实际有值的展示字段数动态调整，1 至 2 项最小宽度为 460 像素，3 至 5 项逐列扩展，超过 5 项保持 880 像素并换行；移除模板固定最小高度和大面积页面留白。
- HTML 图片渲染器新增显式 `data-capture-root` 内容区域截图能力。只有主动标记的模板按元素边界裁剪，其余现有模板继续使用整页截图，避免影响活动日历、物价等图片。
- Playwright 示例覆盖 7 个查询项，验证五列后换行与内容裁剪；生成样例 `target/test-output/html-image/查询信息.png`，尺寸为 `880 × 291`。
### 2026-08-25 角色信息内置查询映射

- 为 `查询信息 区服 角色名 角色信息` 增加不依赖 PostgreSQL 字段配置的内置映射，固定读取 `背包剩余空间`、`角色金币`、`精力`、`侠行点`、`威望`，显示名分别为“背包剩余空间、金币、精力、侠义点、威望”。
- 内置 `角色信息` 映射在数据库字段和分组别名解析前命中，确保新部署无需初始化 `script_status_field` 也可使用；展开出的五个字段仍逐项通过统一查询权限策略。
- 五人日常和门派日常未加入该映射，等待后续确认数据格式后再扩展。
- Playwright 当前角色信息示例仅展示上述五项，输出 `target/test-output/html-image/查询信息.png`，尺寸为 `880 × 206`。
- 本轮全量 `mvn test` 共 677 项通过，失败 0、错误 0、跳过 0；其中 HTML/Vue 浏览器渲染测试 24 项。
### 2026-08-25 Linux nohup 启动脚本

- 项目根目录新增 `start.sh`，使用 POSIX sh 与 `nohup` 启动可执行 JAR，默认端口 8081，日志写入 `logs/botjava.log`，PID 写入 `botjava.pid`。
- 外部配置优先读取脚本同目录 `application.yml`，其次读取 `config/application.yml`；配置或 JAR 不存在时拒绝启动。
- 启动前检查 PID 防止重复运行，自动清理失效 PID；支持通过 `JAR_NAME`、`CONFIG_FILE`、`SERVER_PORT`、`JAVA_BIN`、`JAVA_OPTS`、`LOG_FILE` 和 `PID_FILE` 覆盖部署参数。
### 2026-08-25 查询信息图片文案调整

- `查询信息.html` 不再显示“脚本”相关字样：多条 MongoDB 结果使用“角色记录”分组，底部来源说明改为“数据来自本人已绑定角色”。后台集合查询、权限策略和指令格式不变。
- 图片渲染测试增加模板文案断言，防止后续再次把“脚本”字样带入该截图。
### 2026-08-25 Windows 后台启动脚本

- 项目根目录新增 `start.bat`，供 Windows 常驻主机启动可执行 JAR；外部 YAML 查找顺序、JAR 校验、PID 防重复启动和参数覆盖规则与 `start.sh` 保持一致。
- BAT 通过 PowerShell `Start-Process` 获取真实 Java PID，写入 `botjava.pid`；标准输出写入 `logs\botjava.log`，标准错误写入 `logs\botjava-error.log`。
- 支持 `JAR_NAME`、`CONFIG_FILE`、`SERVER_PORT`、`JAVA_BIN`、`JAVA_OPTS`、`LOG_FILE`、`ERROR_LOG_FILE` 和 `PID_FILE` 环境变量。
- 使用临时无网络 Java JAR 完整验证启动成功、有效 PID 与重复启动保护；PowerShell 启动失败强制返回非零状态，空 PID 和 Windows PID 0 不会被误判为成功。
## 2026-08-25 角色绑定群隔离与参数收敛

- 绑定角色、添加角色统一只接受“区服 + 角色名”，角色列表、MongoDB 归属查询和提示信息不再依赖或展示门派。
- user_role_binding 的业务唯一键固定为 group_open_id + member_openid + server + role_name，同一成员可在不同群绑定相同角色，不同群的角色列表互不串用。
- 绑定角色不会覆盖 user_info.school；默认门派仍作为独立配置保留。
- docs/database/user-info.sql 会按实际约束列删除旧三列唯一约束，再创建群级四列唯一约束。已有数据库即使使用 Hibernate ddl-auto=update 也必须显式执行该脚本。

### 2026-08-25 MongoDB 背包空间预警

- 新增公开群指令 `背包预警`，同时兼容 `/背包预警`；任何群成员都能执行，不校验群权限、发送者角色绑定或 PostgreSQL 用户归属，使用 30 秒外部数据查询冷却。
- `LuaRoleStatusStore` 直接扫描配置的 `roles` 集合，并通过 MongoDB projection 只读取服务器、角色名和背包剩余空间三个字段，显式排除 `_id`；不会读取账号、全局 ID 或其他文档内容。
- Java 兼容数值类型和数字字符串，只保留背包剩余空间小于 50 的记录，并按剩余空间、区服和角色名稳定排序；等于 50、空值和非数字字段不展示。
- 新增可配置字段 `bot.mongodb.role-status.bag-remaining-field`，默认值为 `背包剩余空间`，环境变量为 `BOT_MONGODB_ROLE_STATUS_BAG_REMAINING_FIELD`。
- 新增 Vue 模板 `src/main/resources/static/背包预警.html`：每项标题显示“角色名 - 服务器”，下方显示剩余背包空间；每行最多 5 项，按实际内容区域截图。无命中或 MongoDB 未启用时返回可读文本。
- 新增 Mongo 字段投影、阈值筛选、无角色绑定公开访问、命令规范化与 Playwright 五列换行渲染测试。
- 本轮全量 mvn clean package 共 682 项测试通过，失败 0、错误 0、跳过 0；其中 HTML/Vue 浏览器渲染测试 25 项，已生成可部署 JAR。

### 2026-08-25 定时背包预警推送

- 新增 `SCHEDULED` 任务 `定时背包预警`，稳定编码为 `MONGO_BAG_SPACE_WARNING`；群主或群管理员通过 `开启推送 定时背包预警` 和 `关闭推送 定时背包预警` 管理当前群订阅。
- 新增 `MongoBagSpaceWarningPushTask`，默认 cron 为 `0 0 9 * * *`，未设置 `zone`，因此按运行服务器系统时区每天 09:00 触发；可通过 `BOT_PUSH_MONGO_BAG_WARNING_CRON` 覆盖。
- `ScheduledGroupPushRunner` 新增共享响应入口 `runShared`：调度时只生成一次背包预警图片，再由统一 `GroupPushDispatcher` 投递到全部已订阅群，避免群数量放大 MongoDB 全表读取。
- 交互指令与定时任务复用相同的三字段投影、数值解析、`< 50` 筛选和 `背包预警.html` 模板；交互查询无命中时返回文本，定时任务无命中或查询失败时跳过发送。
- 推送继续受群主动消息总开关、QQ 平台主动消息授权、频控和统一有界线程池约束；未订阅群不会进入 QQ 发送链路。
- 新增任务注册、中文名解析、系统时区 cron、共享响应单次生成、空结果跳过和订阅 Action 回归测试。
- 本轮全量 Maven 构建共 686 项测试通过，失败 0、错误 0、跳过 0；包含 25 项 HTML/Vue 浏览器渲染测试，并重新生成可部署 JAR。

### 2026-08-26 Windows 前台启动脚本

- `start.bat` 从 PowerShell `Start-Process` 后台模式改为当前 CMD 前台执行 `java -jar`；双击后日志直接显示在可见窗口，不再创建 `botjava.pid`，也不再重定向到日志文件。
- Java 退出后显示真实退出码并执行 `pause`，便于观察启动异常或系统终止；测试环境可设置 `NO_PAUSE=1` 跳过暂停。
- 默认 JVM 参数调整为 `-Xms128m -Xmx512m`，仍可通过 `JAVA_OPTS` 覆盖；配置文件继续优先使用 JAR 同目录 `application.yml`，其次使用 `config\application.yml`。
- 针对默认 GBK 的 Windows 系统，脚本通过 `chcp 65001`、`JAVA_ENCODING_OPTS` 和 `logging.charset.console=UTF-8` 统一 CMD、JVM 标准流与 Spring Boot 控制台日志编码；编码参数独立于可覆盖的堆内存参数。
- 保留 `JAR_NAME`、`CONFIG_FILE`、`SERVER_PORT` 和 `JAVA_BIN` 覆盖能力，移除仅服务于后台模式的 PID、标准输出文件和标准错误文件参数。
### 2026-08-26 MongoDB 全库指定字段查询

- 新增公开群指令 `查询全部信息 字段名`，同时兼容 `/查询全部信息 字段名`；不要求用户绑定角色，按 30 秒外部数据查询冷却执行。
- 手动输入的顶层 MongoDB 字段名可直接查询，也复用字段显示名与分组别名展开；所有真实字段在访问数据库前执行内置敏感字段拒绝、配置黑名单和可选白名单校验。
- `LuaRoleStatusStore` 新增字段存在条件与动态 projection，只读取服务器、角色名和目标字段并排除 `_id`；字段不存在或全部没有可展示值时返回明确的无数据文本。
- 新增 Vue 模板 `查询全部信息.html`，每项上方显示“角色名 - 区服”，下方显示字段值，每行最多五项并按实际内容裁剪；列表值继续按多行展示，全库指令不套用单角色查询的 200 项默认限制。
- 新增指令解析、公开访问、敏感字段拦截、空结果、Mongo 投影及 Playwright 五列换行渲染测试。
- 本轮全量 `mvn clean package` 共 693 项测试通过，失败 0、错误 0、跳过 0；其中 HTML/Vue 浏览器渲染测试 26 项，并重新生成可部署 JAR。

### 2026-09-01 全库拥有状态图片优化

- `查询全部信息.html` 对“是”、布尔 `true` 和“已拥有”结果使用翡翠绿色圆形对勾展示；“否”、数字及普通文本仍保留原值，避免改变通用字段查询语义。
- 图片右上角仅在结果包含已拥有项时显示“绿色对勾 + 已拥有”图例，角色数和结果项数继续显示在图例下方。
- 单个拥有状态的值行压缩为 30 像素最小高度，对勾保持 16 像素并居中；角色标题行及多值普通字段布局不变，减少五列卡片换行后的无效留白。
- 服务层模板数据为每个结果项增加 `owned` 标识，并提供顶层 `hasOwnedValues` 控制图例；渲染测试覆盖五列换行、紧凑高度和图例模板契约。
